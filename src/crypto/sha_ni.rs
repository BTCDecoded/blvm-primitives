    ...
    unsafe {
        ...
        // Cached: was std::env::var() per hash — compute_block_tx_ids does ~4000+ sha256/block.
        {
            ...
            if *SHA_NI_DEBUG.get_or_init(|| std::env::var("SHA_NI_DEBUG").is_ok()) {
                ...
                eprintln!(
                    "SHA-NI DEBUG: After Unshuffle - state0 = [{s0_0:08x}, {s0_1:08x}, {s0_2:08x}, {s0_3:08x}]"
                );
                eprintln!(
                    "SHA-NI DEBUG: After Unshuffle - state1 = [{s1_0:08x}, {s1_1:08x}, {s1_2:08x}, {s1_3:08x}]"
                );
                eprintln!(
                    "SHA-NI DEBUG: Expected final hash (empty input): e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"
                );
            }
        }
